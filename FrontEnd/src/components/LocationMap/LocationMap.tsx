...
const LocationMap = ({ location, getLocation }: locationMapProps) => {
  ...
  useEffect(() => {
    ...
    geocoder.geocode({ address: location }, function (result: { geometry: { location: any } }[], status: string) {
      if (status === 'OK') {
        const newLatLng = {
    ...
    function geocodeLatLng(evtLatLng: any) {
      geocoder.geocode({ location: evtLatLng }, function (result: { formatted_address: any }[], status: string) {
        if (status === 'OK') {
    ...
    function geocode(address: string, map: any) {
      geocoder.geocode({ address: address }, function (result: { geometry: { location: any } }[], status: string) {
        if (status === 'OK') {
